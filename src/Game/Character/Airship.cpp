...
void Airship::Update( float dt )
{
    ...
    if ( m_BulletInterval < 0 )
    {
        ...
        m_Bullets.push_back({ 
            // new Bullet( "bullet_2", m_Transform->x + 10, m_Transform->y, -0.5f ), 
            new Bullet( "bullet_1", m_Transform->x + 10, m_Transform->y,  0.0f ), 
            // new Bullet( "bullet_3", m_Transform->x + 10, m_Transform->y,  0.5f ), 
        });
    ...
    for ( size_t i = 0; i < m_Bullets.size(); i++ )
    {
        for ( size_t j = 0; j < m_Bullets[i].size(); j++)
        {
            ...
            // Remove bullet is it's out of scope or hits an object
            if ( m_Bullets[i][j]->GetPos()->y < -20 && i < m_Bullets.size() || m_Bullets[i][j]->GetCollided()  )
            {
